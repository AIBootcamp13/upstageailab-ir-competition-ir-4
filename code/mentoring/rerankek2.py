...
def main():
    ...
    parser.add_argument('--output_path', type=str, required=True, help='결과 JSONL 파일 경로 (sample_submission.csv 포맷)')
    ...
    # 결과 저장용 리스트
    jsonl_lines = []  # sample_submission.csv 형식(JSON Lines)
    score_results = []
    ...
    for query_data in tqdm(queries, desc="쿼리 처리 중"):
        ...
        # sample_submission.csv 형식(JSONL) 한 줄 생성 및 저장 목록에 추가
        jsonl_lines.append({
            'eval_id': eval_id,
            'standalone_query': query_text,
            'topk': [doc_id for doc_id, _ in top_k_docs],
            'answer': "",
            'references': []
        })
    ...
    # 결과를 sample_submission.csv와 동일한 JSONL 포맷으로 저장
    print(f"\n결과를(JSONL: sample_submission.csv 포맷) {args.output_path}에 저장 중...")
    ...
    print("완료!")
    print(f"총 {len(jsonl_lines)}개의 평가 레코드가 생성되었습니다.")
